...
test.before(() => {
  globalThis.context = Mock.context({
    getParkStorage(
      pluginName: string = "CompetitiveMultiplayer"
    ): Configuration {
...
test("getCollection()", (t) => {
  ...
  t.deepEqual(storage.getCollection("foo"), [
    { value: "baz", key: "bar" },
    { value: "strong", key: "ava" },
  ]);
});
...
test("getValueFromCollection()", (t) => {
  ...
  t.deepEqual(storage.getValueFromCollection("foo", "bar"), {
    value: "baz",
    key: "bar",
  });
  t.deepEqual(storage.getValueFromCollection("foo", "ava"), {
    key: "ava",
    value: "strong",
  });
});
...
test("setCollection()", (t) => {
  ...
  storage.setCollection("foo", [
    { key: "bar", value: "baz" },
    { key: "ava", value: "strong" },
  ]);
...
test("parseValueObjectIntoCollection()", (t) => {
  ...
  t.deepEqual(storage.parseValueObjectIntoCollection(valueObject), [
    { value: "bar", key: "baz" },
    {
      value: "strong",
      key: "ava",
