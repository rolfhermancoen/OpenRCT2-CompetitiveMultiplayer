import { ACTION_TYPE, CHEAT_TYPE } from "@lib/enum";

import { Cheater } from "@services/Cheater";
import { Logger } from "@services/Logger";
import { ACTION_QUERY, INTERVAL_TICK, contextSubscribe } from "./Context";
...
export class Utilities {
  ...
  private _disableTrackDesigns(): void {
    contextSubscribe(ACTION_QUERY, (event) => {
      const { player, action } = event;

      if (player.id === -1 && action === ACTION_TYPE.RIDE_CREATE) {
  ...
  private _enableCheats(): void {
    ...
    contextSubscribe(INTERVAL_TICK, () => {
